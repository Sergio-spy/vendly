...
// `version` (write_date compactado) se concatena a la URL para invalidar
// cachés cuando la imagen cambia en Odoo.
function VariantImg({ id, glyph, contain = '80%', imgUrl, imgSize = 128, version }) {
  ...
  const src = imgUrl ? imgUrl(id) : productImageUrl(id, { size: imgSize, v: version });
  return (
    <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', background:'#fff' }}>
      <img
        src={src}
...
export function VariantMosaic({ variantIds = [], fallbackGlyph = 'box', size = '80%', imgUrl, imgSize = 128, version }) {
  ...
  return (
    <div style={{ width:'100%', height:'100%', display:'grid', gridTemplateColumns:'1fr 1fr', gridTemplateRows:'1fr 1fr', gap: 2, background:'#e8e8e8' }}>
      {slice.map((id, i) => (
        <VariantImg key={id ?? `e${i}`} id={id} glyph={fallbackGlyph} contain={size} imgUrl={imgUrl} imgSize={imgSize} version={version}/>
      ))}
    </div>
  );
}
